...
Pour le premier point, nous avons décidé d'opter pour une forme arrondi qui permettrait donc au robot de pouvoir arriver de travers tout en conservant un contact suffisant avec la station de recharge.\\

Pour le second, nous avons décidé que la station de recharge puisse avoir un degré de liberté. Pour réaliser cela, nous avons inséré une partie à l'avant de la station de recharge qui sera soutenu par un support en dessous mais également par des ressors de compression dans les quatre coins de la pièce. \\
...
Une fois la conception 3D effectuée, nous avons dû choisir les procédés de fabrication pour chaque pièce. Après consultation des professeurs du \textit{pôle mécanique de l'ENSTA Bretagne}, nous avons convenu que la partie fixe de la station de recharge qui contient une face incurvée \testit{(élément a)}, serait imprimé en 3D, ainsi que les éléments rectangulaires permettant l'accroche de la partie contacteur sur le robot \testit{(éléments b)}. Tandis que les parties mobiles de la station de recharge \testit{(éléments c)} et la partie contacteur du robot \testit{(éléments d)} seraient elles usinées dans du PVC afin de gagner du temps, d'obtenir des pièces plus rigides et également réduire les coûts de fabrication. \\
